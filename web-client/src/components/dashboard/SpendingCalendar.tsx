...
import { calendarDayLabels, generateCalendarSpendingData } from "../../types/utils/chartUtils"
...
function getAmountTextClass(chargeType: "none" | "canceled" | "active-recurring" | "mixed"): string {
      ...
      <div className="grid grid-cols-7 gap-2">
        {monthSpendingData.map((day) => (
          ...
          >
            ...
            {day.spending > 0 && (
              <p className={`mt-1 text-xs font-medium ${getAmountTextClass(day.chargeType)}`}>{formatCurrency(day.spending)}</p>
            )}
          </div>
        ))}
      </div>
